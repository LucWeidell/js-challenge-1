...
function timeOfDayGreeting(hour) {
        if ((hour >= 5) && (hour <= 11)){
            ...
            return 'Good Night'; //default works assuming int hour given 1-24
    }
}

//4. Write a function that will take in a number and return 'fever' if it indicates a fever (over 98.6)
// and additionally if the person should go to the hospital (at or above 103) 'fever go to hospital'
// (hint: try this with string concatenation), if it is under return 'no fever'
function isFever(temp) {
    let result = '';
    (temp > 98.6) ? result='fever' : result ='no fever'
    if(temp >= 103){
        result = result + ' go to hospital'
    }
    return result;
}
